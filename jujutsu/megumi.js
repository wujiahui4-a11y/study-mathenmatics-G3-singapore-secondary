...
(function () {
  ...
  /* THE SHIKIGAMI THEMSELVES
     The pools stay shadow. The animals that come out of them do not, and
     building all five out of the pool's own near-blacks made five
     silhouettes that could only be told apart by their outline. The Ten
     Shadows read as a pale body with black markings, so that is what they
     are here:

       Divine Dogs    one white, one black, each carrying three dots on
                      the forehead in the other one's colour
       Nue            orange feathers and a white mask of a skull, and
                      the electricity off its wings is purple
       Great Serpent  white, with black markings down its length
       Max Elephant   grey, marked black at the forehead, crown, back
                      and knees
       Rabbits        white, with black ears and eyes                    */
  var FUR_W = 0xe9ecf4, FUR_W_D = 0xc2c8d6;        // the white dog
  /* the black one is black, but not the pool's black: it has to hold a
     shape against a light floor, and its three white dots do the rest */
  var FUR_K = 0x26272f, FUR_K_D = 0x14141d;
  var MARK = 0x0d0d14, MARK_W = 0xf3f5fa;          // the three dots, either way
  var NUE_F = 0xcf7a33, NUE_F_D = 0x9a551d, NUE_BONE = 0xeae4d4;
  var VOLT = 0x9a5cf5, VOLT_L = 0xd8b6ff;          // and the purple off it
  var SNAKE = 0xe3e6ee, SNAKE_D = 0xbabfcd, SNAKE_M = 0x14141f;
  var HIDE = 0xb5bbc7, HIDE_D = 0x8f96a4, HIDE_M = 0x12121b, TUSK = 0xf1ede0;
  var BUN = 0xeef0f6, BUN_D = 0xc7ccd7;
  ...
  function buildDog(white) {
    ...
    /* twins, and the whole point of them is that they are a pair: the
       shaded planes are the darker of the two coats, so a white dog still
       has shape on it rather than being a cut-out */
    var body = white ? FUR_W_D : FUR_K_D;
    var coat = white ? FUR_W : FUR_K;
    var dot = white ? MARK : MARK_W;
    /* chest and haunch */
    ...
    for (var i = 0; i < 5; i++) {
      var tx = -.2 + i * .1;
      part(g, .07, .22, .07, tx, 1.44, 3.6, white ? 0xf6f8fc : 0xdfe4ee);
    }
    /* the three dots on the forehead, in the other one's colour. They are
       the Jewel of Turning Back on the Road on the white one and the Jewel
       of Plenty on the black, and they are how you tell them apart at any
       distance where the coat has gone to grey */
    for (var dd = 0; dd < 3; dd++) {
      part(g, .12, .12, .08, (dd - 1) * .22, 2.16, 2.98, dot);
    }
    ...
    for (s = -1; s <= 1; s += 2) {
      var eye = new THREE.Mesh(new THREE.BoxGeometry(.16, .13, .06),
        new THREE.MeshBasicMaterial({ color: white ? 0x6aa8e8 : 0xff9a6a, toneMapped: false }));
  ...
  function buildNue() {
    var g = new THREE.Group();
    /* an owl in orange feathers wearing a skull for a face — the mask is
       the part everybody draws, so it gets the lightest value on the bird
       and everything else is kept under it */
    part(g, 1.5, 1.2, 3.2, 0, 0, 0, NUE_F);             // body
    part(g, 1.42, .5, 3.0, 0, .38, -.1, NUE_F_D);       // the darker mantle over it
    part(g, .9, .8, 1.1, 0, .3, 1.9, NUE_BONE);         // the skull
    part(g, .96, .24, .5, 0, .66, 2.0, NUE_BONE);       // its brow
    part(g, .34, .3, 1.2, 0, .2, 2.8, NUE_BONE);        // the beak
    /* the human teeth set in it, which is the unpleasant detail */
    for (var t = 0; t < 5; t++) {
      part(g, .09, .13, .07, -.16 + t * .08, .0, 2.9, 0xf6f2e6);
    }
    /* the eye: two holes in the mask with something lit a long way back */
    for (var s = -1; s <= 1; s += 2) {
      part(g, .3, .26, .1, s * .3, .42, 2.32, 0x1a1712);
      var eye = new THREE.Mesh(new THREE.BoxGeometry(.15, .13, .07),
        new THREE.MeshBasicMaterial({ color: 0xffd66a, toneMapped: false }));
      eye.position.set(s * .3, .42, 2.36);
      g.add(eye);
    }
    /* the wings, in three panels each so they can fold. Banded: the
       leading edge stays bright and the flight feathers go dark, so a wing
       reads as a wing rather than as one orange plank */
    g.__wings = [];
    for (s = -1; s <= 1; s += 2) {
      var w = new THREE.Group();
      w.position.set(s * .7, .2, .2);
      part(w, 2.6, .22, 2.2, s * 1.4, 0, 0, NUE_F);
      part(w, 2.6, .12, .6, s * 1.4, .1, .9, NUE_F_D);
      part(w, 2.4, .18, 1.6, s * 3.4, -.1, -.4, NUE_F_D);
      part(w, 2.4, .1, .5, s * 3.4, 0, .3, NUE_F);
      part(w, 1.8, .16, 1.0, s * 5.0, -.2, -.9, 0x74401a);
      g.add(w);
      g.__wings.push(w);
    }
    /* the tail it drags */
    var tail = part(g, .4, .3, 2.4, 0, -.1, -2.4, NUE_F_D);
    tail.rotation.x = .2;
    /* and the talons, which are bone like the face */
    for (s = -1; s <= 1; s += 2) {
      for (var i = 0; i < 3; i++) {
        var c = part(g, .12, .5, .12, s * .45 + (i - 1) * .16, -.8, .4 + i * .1, NUE_BONE);
        c.rotation.x = .4;
      }
    }
    return g;
  ...
  function flyNue(from, dir, ghost) {
    ...
    addFx({ t: 4.5, update: function (dt) {
      ...
      if (!struck) {
        ...
        /* the lightning, down the line it dived on. Purple, because that is
           what comes off its wings — this used to land in the same pale
           blue as everything else Megumi does, so the one shikigami whose
           whole trick is the electricity had no colour of its own. */
        FX.beam(n.position.clone(), new THREE.Vector3(0, -1, 0), 8, VOLT,
          { radius: .5, life: .28 });
        for (var b = 0; b < 7; b++) {
          var bolt = FX.billboard(FX.T.bolt, b % 2 ? VOLT : VOLT_L, .9);
        ...
        FX.flash('#d9beff', .36, .2);
        FX.rings(new THREE.Vector3(at.x, .12, at.z), VOLT, 3, { maxR: 15, life: .5, gap: 44 });
        FX.rings(n.position.clone(), VOLT_L, 2, { maxR: 9, life: .34, ground: false, gap: 40 });
        FX.impact(n.position.clone(), VOLT_L, 3);
        FX.streaks(at.clone().add(new THREE.Vector3(0, 1.4, 0)), VOLT, 9, 20, 1.1);
        FX.speedRing(n.position.clone(), VOLT_L, 13, .26);
        addShake(1.8);
        hitstop(.09);
        ...
        if (!ghost) {
          enemies.forEach(function (e) {
            ...
            e.damage(NUE.dmg, kb, {
              react: 'blow', reactDur: .9, spark: VOLT_L, stun: .9, death: 'burn'
            });
          });
        }
      }
  ...
  function buildSnake() {
    var g = new THREE.Group();
    /* the head. White with black markings, which is the Great Serpent as
       drawn — a black snake coming out of a black pool was invisible right
       up until it had somebody in its mouth */
    part(g, 1.9, 1.5, 3.0, 0, 0, 1.2, SNAKE);
    part(g, 1.5, 1.0, 1.2, 0, -.2, 3.0, SNAKE_D);       // the snout
    part(g, 1.94, .3, 1.8, 0, .64, 1.4, SNAKE_M);       // the band over the crown
    for (var m = -1; m <= 1; m += 2) {
      part(g, .26, .5, 2.6, m * .82, .1, 1.3, SNAKE_M); // and one down each side
    }
    /* the jaw, hinged, so it can open */
    var jaw = new THREE.Group();
    jaw.position.set(0, -.6, .6);
    part(jaw, 1.7, .5, 2.8, 0, 0, 1.2, SNAKE_D);
    for (var i = 0; i < 6; i++) {
      var tx = -.6 + (i % 3) * .6;
      part(jaw, .14, .5, .14, tx, .42, .6 + Math.floor(i / 3) * 1.5, 0xfbfcff);
    }
    g.add(jaw);
    g.__jaw = jaw;
    /* the upper teeth */
    for (i = 0; i < 6; i++) {
      var ux = -.6 + (i % 3) * .6;
      part(g, .14, .55, .14, ux, -.6, 1.4 + Math.floor(i / 3) * 1.5, 0xfbfcff);
    }
    /* the eyes */
    for (var s = -1; s <= 1; s += 2) {
    ...
    for (i = 0; i < 9; i++) {
      var w = 1.9 - i * .13;
      var seg = part(g, w, w, 1.5, 0, 0, -.6 - i * 1.45, SNAKE);
      /* one black band per segment: the markings are what give a long pale
         body any sense of how fast it is going past you */
      part(g, w * 1.04, w * .36, .5, 0, w * .32, -.6 - i * 1.45, SNAKE_M);
      part(g, w * .9, w * .5, 1.5, 0, -w * .34, -.6 - i * 1.45, SNAKE_D);
      g.__segs.push(seg);
    }
    return g;
  }
  ...
  function buildElephant() {
    var g = new THREE.Group();
    /* grey, and marked black at the forehead, the crown, the back and the
       knees. The markings are the whole reason it does not read as one
       enormous dark block from the far side of the arena. */
    part(g, 6.4, 5.2, 9.2, 0, 5.4, 0, HIDE);            // the body
    part(g, 6.5, 1.0, 6.4, 0, 7.6, -.6, HIDE_M);        // the band down its back
    part(g, 4.2, 3.8, 3.4, 0, 6.2, 5.6, HIDE_D);        // the head
    part(g, 4.3, .8, 2.6, 0, 7.7, 5.4, HIDE_M);         // the crown
    part(g, 2.2, 1.4, .3, 0, 6.4, 7.35, HIDE_M);        // and the forehead
    /* the trunk, in seven falling segments */
    g.__trunk = [];
    var ty = 5.2, tz = 7.0;
    for (var i = 0; i < 7; i++) {
      var w = 1.5 - i * .13;
      var seg = part(g, w, w, 1.2, 0, ty, tz, i % 2 ? HIDE_D : HIDE);
      g.__trunk.push(seg);
      ty -= .75; tz += .35;
    }
    /* the tusks */
    for (var s = -1; s <= 1; s += 2) {
      var tu = part(g, .34, .34, 3.2, s * 1.3, 5.0, 6.6, TUSK);
      tu.rotation.x = -.35;
      tu.rotation.y = s * .12;
    }
    /* the ears */
    for (s = -1; s <= 1; s += 2) {
      var ear = part(g, 3.4, 4.0, .3, s * 3.0, 6.4, 4.4, HIDE_D);
      ear.rotation.y = s * .4;
      var rim = part(g, 3.5, .7, .34, s * 3.0, 8.1, 4.4, HIDE_M);
      rim.rotation.y = s * .4;
    }
    /* the eye */
    for (s = -1; s <= 1; s += 2) {
      var eye = new THREE.Mesh(new THREE.BoxGeometry(.3, .26, .1),
        new THREE.MeshBasicMaterial({ color: 0xffca5e, toneMapped: false }));
    ...
    for (i = 0; i < 4; i++) {
      ...
      part(hip, 1.9, 4.4, 1.9, 0, -2.2, 0, HIDE);
      part(hip, 2.0, 1.0, 2.0, 0, -1.1, 0, HIDE_M);     // the black knee
      part(hip, 2.2, .7, 2.2, 0, -4.6, 0, HIDE_D);
      g.add(hip);
      g.__legs.push(hip);
    }
    return g;
  ...
  function buildRabbit() {
    var g = new THREE.Group();
    /* white, with the ears and the eye band black — dozens of these arrive
       at once, so the only thing that matters is that each one is a shape
       against the floor rather than a hole in it */
    part(g, .6, .55, .9, 0, .4, 0, BUN);
    part(g, .62, .2, .7, 0, .62, -.05, BUN_D);
    part(g, .45, .42, .45, 0, .72, .55, BUN);
    for (var s = -1; s <= 1; s += 2) {
      var ear = part(g, .12, .6, .1, s * .13, 1.2, .48, BUN);
      ear.rotation.z = s * .16;
      ear.rotation.x = -.14;
      var tip = part(g, .13, .24, .11, s * .17, 1.44, .44, MARK);
      tip.rotation.z = s * .16;
      tip.rotation.x = -.14;
    }
    part(g, .47, .14, .1, 0, .78, .76, MARK);           // the band over the eyes
    var eye = new THREE.Mesh(new THREE.BoxGeometry(.4, .09, .05),
      new THREE.MeshBasicMaterial({ color: 0xff8a8a, toneMapped: false }));
    eye.position.set(0, .74, .78);
    g.add(eye);
    part(g, .5, .3, .34, 0, .2, -.5, BUN_D);            // the tail end
    return g;
  }
