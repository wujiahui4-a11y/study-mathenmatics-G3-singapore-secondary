...
(function () {
  ...
  function stepRush(dt) {
    ...
    for (var i = 0; i < enemies.length; i++) {
      ...
      kb.y = 17;
      e.damage(14, kb, { react: 'stagger', reactDur: .8, spark: 0x9fd8ff, noFrameBonus: true });
      if (window.JJRAG && !e.net) window.JJRAG.start(e, kb.clone().multiplyScalar(.5));
  ...
  function endCine() {
    ...
    A.release(); V.release();
    CINE.on = false;
    /* the fist finally arrives */
    if (A.self && V.ent && V.ent.damage) {
      var dir = new THREE.Vector3(Math.sin(player.facing), 0, Math.cos(player.facing));
      V.ent.damage(58, dir.multiplyScalar(26).setY(15),
        { react: 'stagger', reactDur: .9, spark: 0x9fd8ff, noFrameBonus: true });
    }
    if (A.self && window.JJAW) window.JJAW.finish();
    if (V.self) { player.iframes = 1.2; }
  }
  ...
  function buildSlab() {
    ...
    var g = new THREE.Mesh(new THREE.BoxGeometry(13, 2.2, 150),
  ...
  { dur: 3.0, enter: function () {
    ...
    step: function (t) {
      ...
      poseFly(V.rig, 1);
      V.rig.root.position.set(STAGE.x, STAGE.y + Math.max(1.4, vy), STAGE.z);
      poseStamp(A.rig, 1);
      A.rig.root.position.set(STAGE.x, STAGE.y + Math.max(3.4, ay), STAGE.z);
      A.rig.root.rotation.set(0, Math.PI, 0);
  ...
  { dur: 2.9, enter: function () {
    ...
    step: function (t) {
      var A = CINE.A, V = CINE.V;
      /* the victim getting back up, slowly */
      var up = Math.min(1, t / 1.5);
      poseLimp(V.rig, t);
      V.rig.spine.rotation.x = .55 - .35 * up;
      V.rig.neck.rotation.x = .45 - .3 * up;
      V.rig.hips.position.y = V.rig.hipsBaseY - .3 + .3 * up;
      V.rig.root.position.set(STAGE.x, STAGE.y, STAGE.z);
      V.rig.root.rotation.y = t * .35;
