...
(function () {
  ...
  { dur: 4.2, enter: function () {},
    step: function (t) {
      ...
      shot(t, [
        ...
        { t: 1.9, p: [-14.5, 4.4, 8], l: [0, 2.3, 6.4] },
        { t: 2.6, p: [-13, 3.6, 7.6], l: [0, 1.9, 6.6] },
        { t: 4.2, p: [-14, 1.2, 7], l: [0, -5, 6.6] }
      ]);
      if (t > 2.62) {
        once('crack', function () {
          FX.flash('#fff3e0', .5, .3);
  ...
  { dur: 3.0, enter: function () {
    ...
    step: function (t) {
      ...
      if (ay > 6) {                                   // still coming down
        rp(A.rig);
        A.rig.spine.rotation.x = .3;
        A.rig.neck.rotation.x = -.35;
        A.rig.shoulderL.rotation.x = -2.4; A.rig.shoulderR.rotation.x = -2.5;
        A.rig.shoulderL.rotation.z = -.6; A.rig.shoulderR.rotation.z = .6;
        A.rig.elbowL.rotation.x = -.5; A.rig.elbowR.rotation.x = -.45;
        A.rig.hipL.rotation.x = -1.1; A.rig.kneeL.rotation.x = 1.5;
        A.rig.hipR.rotation.x = -.5; A.rig.kneeR.rotation.x = .9;
        A.rig.hips.position.y = A.rig.hipsBaseY - .2;
      } else {
        poseStamp(A.rig, 1);
      }
  ...
  { dur: 2.9, enter: function () {
    ...
    step: function (t) {
      ...
      var x = Math.min(1, t / 2.2), up = x * x * (3 - 2 * x);
      if (up < 1) {
        poseDown(V.rig);
  ...
  function skipToBeat(n) {
    ...
    n = (n | 0) - 1;                               // 1-based, as the cut is numbered
  ...
  if (window.__game) {
    ...
    window.__game.castLimitless = castLimitless;
    window.__game.castN1 = castN1;
