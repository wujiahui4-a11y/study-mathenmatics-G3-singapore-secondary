...
(function () {
  ...
  function stepSong(dt) {
    if (!songFade || !songEl) return;
    songEl.volume = Math.max(0, songEl.volume - dt * .9);
    if (songEl.volume <= .02) {
      try { songEl.pause(); songEl.currentTime = 0; } catch (e) {}
      songEl.volume = SONG_VOL;
      songFade = 0;
    }
  }

  /* on: a Gojo in the room just started the entrance. off: that Gojo
     is no longer awakened. The track restarts for a new entrance and
     only fades when nobody in the room is still in it. */
  function theme(on, id) {
    id = id || 'local';
    if (on) {
      var first = !songIds[id];
      songIds[id] = 1;
      if (first || songEl && (songEl.paused || songFade)) playSong();
    } else {
      delete songIds[id];
