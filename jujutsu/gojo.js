...
(function () {
  ...
  function ensureSong() {
    ...
    a.addEventListener('error', function () {
      ...
      if (songUrlI >= songUrls.length) {
        songErr = 'could not load the theme';
        if (window.JJNOTICE) window.JJNOTICE('THEME FAILED TO LOAD', '#ff8fa5');
        return;
      }
      songErr = 'try ' + songUrls[songUrlI];
      a.src = songUrls[songUrlI];
      a.load();
      if (Object.keys(songIds).length) kickPlay(a);
    });
    songEl = a;
    return a;
  }
  ...
  function stepSong(dt) {
    ...
    if (songFade) {
      ...
      songWant = want;
      if (songEl && (songEl.paused || songEl.muted)) playSong(false);
    } else {
      want = 0;
      songWant = 0;
    }
  ...
  function theme(on, id) {
    ...
    if (on) {
      ...
      songIds[id] = 1;
      playSong(!!first);
      if (first && window.JJNOTICE) window.JJNOTICE('FINAL ENCORE', '#9fd8ff');
    } else {
      delete songIds[id];
  ...
  AW.debugHear = function () {
    ...
    return {
      ...
      vol: songEl ? +songEl.volume.toFixed(3) : 0,
      muted: !!(songEl && songEl.muted),
      t: songEl ? +songEl.currentTime.toFixed(2) : 0,
      paused: !songEl || songEl.paused,
      file: songEl ? songEl.currentSrc : '',
      err: songErr
    };
  };
