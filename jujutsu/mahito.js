...
(function () {
  ...
  var KIT = (M.kit = {
    mh_stock: { name: 'Stockpile', cd: 12, dur: 1.05, damage: 13, slot: 'mh1' },
    mh_air: { name: 'Aerial Stockpile', cd: 12, dur: 1.3, damage: 12, slot: 'mh1' },
    mh_fire: { name: 'Soul Fire', cd: 12, dur: 1.5, damage: 13, slot: 'mh2' },
    mh_focus: { name: 'Focus Strike', cd: 15, dur: 1.15, damage: 14, slot: 'mh3' },
    mh_black: { name: 'Focus Strike · Black Flash', cd: 15, dur: 1.15, damage: 15, slot: 'mh3' },
    mh_chain: { name: 'Chainwhip', cd: 15, dur: 1.3, damage: 3, slot: 'mh3' },
    mh_home: { name: 'Homerun', cd: 15, dur: 1.4, damage: 15, slot: 'mh3' },
    mh_repel: { name: 'Body Repel', cd: 20, dur: 0.95, damage: 13, slot: 'mh4' },
    mh_ride: { name: 'Body Repel · Ride', cd: 20, dur: 2.95, damage: 13, slot: 'mh4' },
    ...
    effect(
      ...
      (t, dt, o) => {
        ...
        if (owner !== false) {
          ...
          if (targets.length) {
            var e = targets[0];
            hit(e, 'mh_fire', 4.5, d, index >= 2 ? 21 : 1, index >= 2 ? 6 : 0, {
      ...
      (t, dt, o) => {
        ...
        if (owner !== false) {
          lineTargets(prev, d, step + 2, 2.5, -2, 4).forEach((e) => {
            if (!caught.has(e) && hit(e, 'mh_repel', 13, d, 20, 8, { unblockable: true })) caught.add(e);
          });
          caught.forEach((e) => {
  ...
  function step(a, dt) {
    ...
    if (type === 'mh_stock') {
      ...
      beat('two', 0.68, () => {
        ...
        es.forEach((e) => hit(e, type, 7, d, 12, 12, { unblockable: true }));
      });
    } else if (type === 'mh_air') {
      if (a.t < 0.45) moveActor(player, d, dt * 7);
      beat('slam', 0.5, () => {
        player.vel.y = -22;
        front(6, 3).forEach((e) => hit(e, type, 12, d, 8, -10, { unblockable: true }));
