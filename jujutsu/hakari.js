  ...
  function doorMaps() {
    ...
    DOOR_SLAT = doorCanvas(16, 64, function (x, w, h) {
      ...
      for (i = 0; i < 8; i++) {
        ...
        g.addColorStop(0, '#d4dbe4');
        g.addColorStop(.28, '#9aa4b2');
        g.addColorStop(.5, '#5c6574');
        g.addColorStop(.72, '#b4bcc8');
        g.addColorStop(1, '#6a7382');
        x.fillStyle = g;
  ...
  function makeDoor() {
    ...
    function mat(c, opt) {
      ...
      var m = new THREE.MeshStandardMaterial({
        color: c,
        roughness: opt.rough == null ? .42 : opt.rough,
        metalness: opt.metal == null ? .72 : opt.metal,
    ...
    var steel = mat(0xb8c2ce, { rough: .38, metal: .78 });
    var steelD = mat(0x5a6270, { rough: .48, metal: .7 });
    var steelR = mat(0xe8eef4, { rough: .34, metal: .82, map: DOOR_SLAT });
    var ink = mat(0x1a1c22, { rough: .88, metal: .08 });
    var gold = mat(0xffd24a, { rough: .26, metal: .88, emit: 0xffaa22, emitI: .42, gold: 1 });
    var goldH = mat(0xffe08a, { rough: .2, metal: .9, emit: 0xffc44d, emitI: .7, gold: 1 });
    var pink = mat(0xff4f8b, { rough: .4, metal: .35, emit: 0xff2a6a, emitI: .28, gold: 1 });
    var glass = mat(0xffffff, { rough: .08, metal: .25, map: DOOR_GLASS, alpha: .82 });
    var plate = mat(0xffffff, { rough: .45, metal: .2, map: DOOR_PLATE });
    var hazY = mat(0xffcc33, { rough: .55, metal: .25, emit: 0xaa8800, emitI: .12 });
    var hazK = mat(0x14161a, { rough: .8, metal: .1 });

    /* the frame the shutter rides in */
    box(g, 7.55, .42, .86, steel, 0, 3.22, 0);
    box(g, 7.7, .12, .94, gold, 0, 3.46, 0);
    box(g, .38, 6.6, .86, steel, -3.58, 0, 0);
    box(g, .38, 6.6, .86, steel, 3.58, 0, 0);
    box(g, 7.55, .34, .92, steelD, 0, -3.28, 0);
    box(g, 7.2, 6.35, .16, steelD, 0, 0, -.28);     // the back plate
    box(g, 7.35, 6.5, .1, ink, 0, 0, -.22);          // the well it sits in

    /* gold cursed-energy rim, slightly proud of the steel */
    box(g, 7.85, .07, .07, goldH, 0, 3.58, .38);
    box(g, 7.85, .07, .07, goldH, 0, -3.48, .38);
    box(g, .07, 7.1, .07, goldH, -3.82, 0, .38);
    box(g, .07, 7.1, .07, goldH, 3.82, 0, .38);

    var side;
    for (side = -1; side <= 1; side += 2) {
      var leaf = new THREE.Group();
      leaf.position.set(side * 1.72, 0, .06);
      g.add(leaf);

      box(leaf, 3.28, 6.05, .22, steel, 0, 0, -.04);
      /* corrugated slats — the shutter itself */
      var s;
      for (s = 0; s < 10; s++) {
        var sy = -2.62 + s * .38;
        var slat = box(leaf, 3.12, .3, .42, steelR, 0, sy, .12);
        slat.userData.home = slat.position.clone();
        g.userData.slats.push(slat);
        box(leaf, 3.12, .06, .46, steelD, 0, sy - .16, .14);
      }
      /* the window in the top third, with the 777 on the glass */
      box(leaf, 2.55, 1.85, .1, ink, 0, 2.08, .16);
      box(leaf, 2.36, 1.66, .06, glass, 0, 2.08, .22);
      box(leaf, 2.58, .1, .16, gold, 0, 2.96, .24);
      box(leaf, 2.58, .1, .16, gold, 0, 1.2, .24);
      box(leaf, .1, 1.86, .16, gold, -1.24, 2.08, .24);
      box(leaf, .1, 1.86, .16, gold, 1.24, 2.08, .24);
      box(leaf, 2.4, .06, .12, gold, 0, 2.08, .26);
      box(leaf, .06, 1.66, .12, gold, 0, 2.08, .26);

      /* the line across it that belongs to the train */
      box(leaf, 3.16, .28, .14, plate, 0, 1.02, .28);
      box(leaf, 3.16, .05, .16, pink, 0, 1.18, .3);

      /* grab bar, and a lamp over the glass */
      box(leaf, .08, .72, .08, ink, side * -.9, -.15, .36);
      box(leaf, .34, .08, .08, ink, side * -.9, -.15, .36);
      box(leaf, .22, .1, .12, goldH, 0, 2.88, .34);

      /* outer stiffener */
      box(leaf, .12, 6.0, .48, steel, side * 1.52, 0, .1);
    }

    /* the meeting stile, rubber and latch */
    box(g, .18, 6.15, .62, steelD, 0, 0, .16);
    box(g, .1, 6.05, .68, ink, 0, 0, .2);
    box(g, .34, .42, .28, gold, 0, .15, .42);
    box(g, .14, .18, .16, ink, 0, .15, .52);

    /* rubber sweep, and the hazard blocks on the sill */
    box(g, 7.1, .16, .7, ink, 0, -3.08, .18);
    var h;
    for (h = 0; h < 9; h++) {
      box(g, .72, .12, .78, h % 2 ? hazY : hazK, -2.88 + h * .72, -3.38, .2);
    }

    /* rivets along the header and the posts */
    var r, rv = mat(0x9aa3b0, { rough: .3, metal: .88 });
    for (r = -3; r <= 3; r++) {
      box(g, .1, .1, .12, rv, r * 1.05, 3.22, .46);
    }
    for (r = -2; r <= 2; r++) {
      box(g, .1, .1, .12, rv, -3.58, r * 1.15, .46);
      box(g, .1, .1, .12, rv, 3.58, r * 1.15, .46);
    }

    /* cursed-energy rings, front and back, so it reads when it flies */
    var glow = FX.billboard(FX.T.ring, 0xffcc4d, .55);
    glow.scale.set(9.4, 9.4, 1);
    glow.position.z = .62;
    g.add(glow);
    g.userData.glow = glow;
    var glowB = FX.billboard(FX.T.ring, 0xff4f8b, .32);
    glowB.scale.set(8.2, 8.2, 1);
    glowB.position.z = -.42;
    g.add(glowB);
    g.userData.glowB = glowB;
    var star = FX.billboard(FX.T.star, 0xffe08a, .4);
    star.scale.set(3.2, 3.2, 1);
    star.position.set(0, .15, .7);
    g.add(star);
