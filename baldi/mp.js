...
(function () {
  ...
  function startMatch() {
    ...
    MP.lockAttempts = 0; MP.triedFullscreen = false;
  ...
  function lockWatch() {
    ...
    document.body.classList.toggle('mpMouseLocked', locked);
  ...
  function bindLook() {
    ...
    document.addEventListener('pointerlockerror', function (e) {
      if (!MP.active) return;
      /* Stop the original handler from enabling edge-of-screen look. Strict
         multiplayer look remains blocked until pointer lock succeeds. */
    ...
    /* The original game's unlocked handler rotates from cursor movement and
       eventually stops at the screen edge. Suppress it completely. */
