...
(function () {
  ...
  function startMatch() {
    ...
    MP.lockFails = 0; MP.lockGaveUp = false; MP.triedFullscreen = false;
  ...
  /* ---------------------------------------------------------------- input */
  /* ------------------------------------------------------------ mouse look
     The game asks for pointer lock once, programmatically, right after a level
     loads. A browser refuses a lock that is not tied to a click, and the game
     then latches lockDenied and never asks again — which leaves you turning by
     shoving the cursor at the edge of the screen for the rest of the match.
     Here it is asked for properly on a click, retried, and only given up on
     after a few genuine refusals. */
  ...
  function grabMouse() {
    ...
    try { pr = view.requestPointerLock({ unadjustedMovement: true }); } catch (e) { pr = null; }
    if (pr === undefined) return;                       // old API, events will tell us
    if (!pr) { try { view.requestPointerLock(); } catch (e2) {} return; }
    if (pr.catch) {
      pr.catch(function () {
        var p2;
  ...
  function lockWatch() {
    if (!MP.active) { el('mpClick').style.display = 'none'; return; }
    if (G.paused) {
      G.paused = false;
  ...
  function bindLook() {
    ...
    document.addEventListener('pointerlockchange', function () {
      if (pointerIsLocked()) { MP.lockFails = 0; MP.lockGaveUp = false; }
      lockWatch();
    });
    document.addEventListener('pointerlockerror', function () {
      if (!MP.active) return;
      MP.lockFails = (MP.lockFails || 0) + 1;
      /* three real refusals: stop fighting it and hand back the game's own
         look-by-moving fallback so the player is never stuck */
      if (MP.lockFails >= 3) {
        MP.lockGaveUp = true;
        try { lockDenied = true; } catch (e) {}
        var lh = UI.el('lookHint');
        if (lh) lh.classList.remove('hidden');
        if (UI) UI.say('This page will not lock the mouse — move it to look instead.', 5000);
      }
      lockWatch();
    });
  }
