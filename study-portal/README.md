# Study Portal — Group Study Room

A 2D top-down multiplayer arena that lives inside a page that looks like an
ordinary mathematics revision site. Everybody starts with a plain bullet; every
time you defeat someone you gain a level and choose a new skill.

* **Levels cost 1, 2, 4, 8, 16 … kills.** Level 1 → 2 needs one defeat, 2 → 3
...
Open the page, choose **Enter Group Study Room → Create room**. You get a code
such as `AK7QM4` and a link like:

```
https://<wherever-this-is-hosted>/study-portal/index.html?room=AK7QM4
```
...
returned to the lobby and a new room has to be created.
...
| Hide the game instantly | `Esc` (shows revision notes; `Esc` again returns) |
...
```
studyportal/arena3/<ROOM>/h    host -> everyone (snapshots, events)
studyportal/arena3/<ROOM>/c    everyone -> host (input, joins, skill picks)
```
...
| File | What it does |
| --- | --- |
| `index.html` | the study-site shell, lobby, HUD and revision-notes overlay |
| `css/app.css` | styles for both the site layer and the game HUD |
...
| `js/ui.js` | DOM: lobby, HUD, cards, kill feed |
...
# then open http://localhost:8080/study-portal/index.html
```

Opening `index.html` straight from disk also works for solo practice.
